	...
	public function postSave()
	{
		...
		foreach($owners as $owner)
		{
			if($owner->getId() !== Core::getUser()->getId())
				Message::create(UserAccount::get(UserAccount::ID_SYSTEM_ACCOUNT), $owner, 'Users changed for Property: ' . $property->getAddress(), str_replace('{roleName}', 'Owner', $message), Message::TYPE_EMAIL);
		}
		...
		foreach($agents as $agent)
		{
			if($agent->getId() !== Core::getUser()->getId())
				Message::create(UserAccount::get(UserAccount::ID_SYSTEM_ACCOUNT), $agent, 'Users changed for Property: ' . $property->getAddress(), str_replace('{roleName}', 'Agent', $message), Message::TYPE_EMAIL);
		}
	}
    /**
     * (non-PHPdoc)
